...
mapa = leaflet() |> 
  ...
  addLegendNumeric(
    pal = paleta_infraestructura, 
    values = raster::values(infraestructura)[!is.na(raster::values(infraestructura))],  
    title = "Percepcion infraestructura vial", position = "bottomright", group = "Percepcion infraestructura vial",
    orientation = 'horizontal', shape = 'rect', decreasing = FALSE, height = 20, width = 200,
    labels = c("Mala", "Buena"), tickLength = 0
  ) |> 
  addRasterImage(viajes, opacity = 0.7, group = "Nivel de uso por numero de viaje", colors = paleta_viajes) |> 
  ...
  addRasterImage(correlacion_pearson_7, opacity = 0.7, group = "Correlacion Pearson", colors = paleta_correlacion) |> 
  addLegendNumeric(
    pal = paleta_correlacion, 
    values = raster::values(correlacion_pearson_7)[!is.na(raster::values(correlacion_pearson_7))],  
  ...
  addLayersControl(
    overlayGroups = c("Percepcion infraestructura vial", "Nivel de uso por numero de viaje", "Correlacion Pearson"),
    options = layersControlOptions(collapsed = FALSE)
  ) |> 
  hideGroup(c("Nivel de uso por numero de viaje", "Correlacion Pearson"))  
